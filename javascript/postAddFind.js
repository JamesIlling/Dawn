...
$(document).ready(function () {
    $("form[name=frmAddFind]").submit(function (eventobj) {
        ...
        var id = json['data'][0]['id'];
            ...
            if (id == '') {
                method = "POST";
                path = getServiceLocation() + "finds";
            }
            else {
                method = "PUT";
                path = getServiceLocation() + "finds/" + id;
            }
...
function GetDataFromForm() {
    var data = '{"data":[';
    $(":input:not(:button):not(:submit)").each(function () {
        if ($(this).closest('[id]').attr('id') != 'sites') {
